...
func (c *ProtobufClient) Receive(receiveChannel chan dht.Packet) {
	readedUDPMessage := make(chan *core.Protocol)
...
func (c *ProtobufClient) handleDeserializeData(readMessage <-chan *core.Protocol, receiveChannel chan<- dht.Packet, conn net.Conn) {
	for {
